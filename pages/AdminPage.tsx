...
const AdminPage: React.FC<AdminPageProps> = ({ 
  ...
  // Statistik Logic - Menyesuaikan dengan Jenis Pekerjaan baru
  const workTypes = ['JTM Tier 1', 'JTM Tier 1 - Tier 2', 'GARDU Tier 1', 'GARDU Tier 1 - Tier 2'];
  const statsWorkByUlp = ulpList.map(u => {
    const result: any = { name: u.name.replace('ULP ', '') };
    workTypes.forEach(w => {
      // Menghitung jumlah temuan berdasarkan kategori pekerjaan baru
             ...
             <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={statsWorkByUlp} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <Bar dataKey="JTM Tier 1" fill="#6366F1" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="JTM Tier 1 - Tier 2" fill="#818CF8" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="GARDU Tier 1" fill="#F59E0B" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="GARDU Tier 1 - Tier 2" fill="#D97706" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
             </div>
          </div>

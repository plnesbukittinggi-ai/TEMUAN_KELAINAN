...
/**
 * Membersihkan link Google Maps atau teks Geotag menjadi format koordinat Lat, Long.
 */
const extractCoordinates = (geotag: string): string => {
  ...
  // 1. Pola koordinat standar: -1.234, 101.234
  const coordRegex = /(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)/;
  const match = str.match(coordRegex);
  if (match) return `${match[1]}, ${match[2]}`;

  // 2. Pola @lat,long (sering ada di URL Google Maps)
  ...
  // 4. Bersihkan dari simbol dan link jika pola tidak ditemukan
  ...
  async downloadPDF(data: TemuanData[], filters: any) {
    ...
    const body = data.map((item, i) => {
      ...
      return [ 
        ...
        extractCoordinates(item.geotag || ""), '', '', item.keterangan, displayStatus 
      ];
    });
